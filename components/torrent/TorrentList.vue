<template>
  <div>
    <div class="p-2 flex flex-col border-2 border-base-content/20 rounded-2xl grow w-full">
      <div class="flex flex-col">
        <a
          v-for="(torrent, index) in torrents"
          :key="index"
          class="flex flex-col rounded-2xl hover:bg-gradient-to-t from-base-300/30 to-base-300/20 text-sm cursor-pointer duration-200 group"
        ...
        >
          <div class="px-4 pt-4 pb-4 group-hover:pb-2 flex flex-row flex-nowrap justify-start items-center rounded-2xl w-full">
            <div class="flex flex-col flex-nowrap justify-start items-center font-semibold overflow-hidden grow">
              <span class="whitespace-nowrap text-ellipsis group-hover:text-[0.95rem] text-neutral-content overflow-hidden w-full duration-200">{{ torrent.title }}</span>
              <div class="mt-1 flex flex-row flex-nowrap justify-start items-start w-full">
                ...
                <a class="ml-2 whitespace-nowrap text-xs text-primary">u/{{ torrent.uploader }}</a>
              </div>
            </div>
            <div class="flex flex-col flex-nowrap justify-center items-center">
              <div class="flex flex-row flex-nowrap text-center font-semibold rounded-2xl">
                <div class="w-10 h-10 flex flex-col shrink-0 justify-center text-green-500 border-2 border-base-content/20 group-hover:border-base-100 rounded-2xl duration-200">{{ torrent.seeders }}</div>
                <div class="ml-2 w-10 h-10 flex flex-col shrink-0 justify-center text-red-500 border-2 border-base-content/20 group-hover:border-base-100 rounded-2xl duration-200">{{ torrent.leechers }}</div>
                <div class="ml-2 w-10 h-10 flex flex-col shrink-0 items-center justify-center text-neutral-content/50 hover:text-neutral-content bg-base-300 group-hover:bg-base-300 rounded-2xl duration-200">
